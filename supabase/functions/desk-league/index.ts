...
// Bronze), each one frontier model that decides and four worker models that
// do the brute work, each with its own $100k paper book. Every candidate the
// scan flags goes to every live team: the workers research and vote, the
// frontier decides, code sizes and writes the trade with a ticket. Three
// sessions a day the workers propose from the feed and the frontier reviews
// the book. A team dies 5% below its start, or as the worst in Bronze at the
// daily cut; a replacement forms from a set of models never used before. The
// council (frontier + two senior workers) can kick a member daily.
//
// Every stage that needs more than one model call is a child invocation
// (mode decide / session_team / council_team) launched by the tick and left
// to finish on its own: the gateway cuts any invocation at 150s.
//
...
import { leagueSettings, draftTeams, replacementTeam, rankTiers, teamName, poolStanding, deathLine, sessionDue, comboKey, isPassive, rankScore, type LeagueSettings, type TeamLike, type Tier } from "./lib/league.ts";
...
async function saveOpinion(uid: string, decisionId: string | null, model: string, juror: string, round: string, res: Res, content: J): Promise<void> {
  await rest("desk_opinions", { method: "POST", body: JSON.stringify({ user_id: uid, session_id: null, sit_id: null, decision_id: decisionId, model, juror, round, content, raw: res.raw.slice(0, 4000), latency_ms: res.latency, cost_usd: res.cost, tokens_in: res.tokensIn, tokens_out: res.tokensOut, error: res.error }) });
}
...
function workerSystem(team: TeamRow, b: BookState, s: LeagueSettings): string {
  const research = s.research === "light";
  return `You are a worker on team "${team.name}" in the ${team.tier} league of a paper-trading tournament run by Ben, 19, who is learning markets by watching you. The frontier decides; you do the brute work. A coded strategy has flagged a setup. Check the mechanism (does the reason for the move hold up?), the level (is the entry good here, or has the move already happened?), the timing (an event or headline that changes it) and the size of the risk.${research ? " You may look things up first: candles at another interval, the funding rate, a news search, the strategy's record." : ""}
Vote "take" or "pass" with your confidence 0-1 that the target is hit before the stop within the horizon. Your vote and your confidence are scored against what the strategy's own rule-only book does with this exact setup, so passing on winners costs you as much as taking losers. Say what you believe.
${COMPETITION(s)}
You may tighten the stop or target and lower the leverage; you may not widen the stop or change the symbol (return the setup's own numbers when you change nothing). Thesis in at most 60 words; "wrong_if" is one observable thing. Tags from: chase, extended, no-catalyst, event-risk, crowded, thin-volume, counter-trend, clean, strong-confluence.
${bookText(b)}
...
async function spentToday(uid: string, day: string): Promise<number> {
  const r = rows(await rest(`desk_opinions?user_id=eq.${uid}&decision_id=not.is.null&created_at=gte.${day}T04:00:00Z&select=cost_usd&limit=5000`));
  return r.reduce((a, o) => a + num(o.cost_usd), 0);
}
async function insertTeam(uid: string, s: LeagueSettings, all: TeamRow[], pick: { frontier: string; workers: string[]; combo: string }, tier: Tier, season: number, why: string): Promise<TeamRow | null> {
  const ordinal = all.filter((t) => t.frontier === pick.frontier).length + 1;
  const name = teamName(pick.frontier, ordinal);
  const made = rows(await rest("desk_teams", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ user_id: uid, name, frontier: pick.frontier, workers: pick.workers, seniors: pick.workers.slice(0, 2), combo: pick.combo, tier, status: "live", season, start_equity: START, equity: START, peak: START, return_pct: 0, stats: {} }) }))[0];
  if (!made) return null;
  await log(uid, name, "formed", pick.frontier, pick.workers.join(", "), why);
...
function briefText(b: J, book: BookState, rules: Rules): string {
...
function toolsFor(uid: string, setup: J, cache: BarCache, feed: J[], funding: Record<string, number>, record: string, book: BookState): Tool[] {
...
async function workerBallot(uid: string, key: string, team: TeamRow, model: string, decisionId: string, brief: J, book: BookState, rules: Rules, s: LeagueSettings, tools: Tool[], deadline: number): Promise<Ballot> {
  ...
  await saveOpinion(uid, decisionId, model, "worker", "worker", res, res.json ? { ...ballot } : {});
  return ballot;
}
...
async function cycle(uid: string, body: J): Promise<J> {
  ...
  // stale children: launched more than four minutes ago and still not done → one relaunch, then failed
  const stale = rows(await rest(`desk_decisions?user_id=eq.${uid}&status=eq.launched&updated_at=lt.${iso(t0 - 240_000)}&select=id,team_id,launched&limit=100`));
  const relaunch: Record<string, string[]> = {};
  for (const d of stale) {
    const l = (d.launched as J) ?? {};
    if (num(l.n) < 2) { (relaunch[String(d.team_id)] ??= []).push(String(d.id)); await rest(`desk_decisions?id=eq.${d.id}`, { method: "PATCH", body: JSON.stringify({ launched: { at: t0, n: num(l.n) + 1 }, updated_at: iso(t0) }) }); (out.relaunched as number)++; }
    else { await rest(`desk_decisions?id=eq.${d.id}`, { method: "PATCH", body: JSON.stringify({ status: "failed", outcome: { taken: false, reasons: ["the team did not answer in time (two tries)"], pass_reason: "no answer in time", by: "failed" }, updated_at: iso(t0) }) }); (out.failed as number)++; }
  }
  for (const [teamId, ids] of Object.entries(relaunch)) launch({ mode: "decide", userId: uid, team_id: teamId, decision_ids: ids });
  // new candidates: every fresh setup goes to every live team
  const fresh = rows(await rest(`desk_setups?user_id=eq.${uid}&status=eq.new&expires_at=gte.${iso(t0)}&select=*&order=score.desc,created_at.desc&limit=10`));
  if (fresh.length) {
    ...
    for (const su of fresh) {
      ...
      const batch = live().map((t) => ({ user_id: uid, team_id: t.id, kind: "candidate", setup_id: su.id, symbol: su.symbol, strategy: su.strategy, timeframe: su.timeframe, status: "queued", brief, ballots: [], launched: {} }));
  ...
  // dispatch: up to four queued candidates per team per tick, one child per team
  const spent = await spentToday(uid, today);
  out.spent_today = round(spent, 3);
  const queued = rows(await rest(`desk_decisions?user_id=eq.${uid}&status=eq.queued&select=id,team_id&order=created_at.asc&limit=200`));
  const byTeam: Record<string, string[]> = {};
  for (const d of queued) (byTeam[String(d.team_id)] ??= []).push(String(d.id));
  for (const team of live()) {
    const ids = (byTeam[team.id] ?? []).slice(0, 4);
    if (!ids.length) continue;
    if (spent >= s.budget_usd_day) {
      for (const id of ids) await rest(`desk_decisions?id=eq.${id}`, { method: "PATCH", body: JSON.stringify({ status: "done", outcome: { taken: false, reasons: [`today's league budget ($${s.budget_usd_day}) is spent`], pass_reason: "budget spent", by: "budget" }, updated_at: iso(t0) }) });
      (out.skipped as string[]).push(`${team.name}: budget`);
      continue;
    }
    await rest(`desk_decisions?id=in.(${ids.join(",")})`, { method: "PATCH", body: JSON.stringify({ status: "launched", launched: { at: t0, n: 1 }, updated_at: iso(t0) }) });
    launch({ mode: "decide", userId: uid, team_id: team.id, decision_ids: ids });
    (out.launched as number) += ids.length;
  }
  if (body.dispatch_only !== true) {
    const now = etParts(t0);
    const due = sessionDue(now.hour, now.minute, s.session_times);
    if (due) launch({ mode: "session", userId: uid, time: due });
    if (now.hour * 60 + now.minute >= 16 * 60 + 6) launch({ mode: "council", userId: uid });
  }
  out.ms = Date.now() - t0;
  return out;
}
...
async function decide(uid: string, body: J): Promise<J> {
  ...
  if (!acct || !team) return { error: "no team" };
  const s = leagueSettings(acct.league as Partial<LeagueSettings>);
  const key = (await secret("anthropic_api_key")) || ENV_KEY;
  if (!key) return { error: "no key" };
  const decisions = rows(await rest(`desk_decisions?user_id=eq.${uid}&team_id=eq.${team.id}&id=in.(${ids.join(",")})&status=eq.launched&select=*`));
  if (!decisions.length) return { ok: true, note: "nothing launched" };
  const rules = rulesFor(String(acct.preset ?? "aggressive") as "no_limits", (acct.rules as Partial<Rules>) ?? {});
  const finish = async (d: J, patch: J) => rest(`desk_decisions?id=eq.${d.id}`, { method: "PATCH", body: JSON.stringify({ ...patch, updated_at: new Date().toISOString() }) });
  if (team.status !== "live") { for (const d of decisions) await finish(d, { status: "done", outcome: { taken: false, reasons: ["the team is dead"], pass_reason: "the team died", by: "guardrail" } }); return { ok: true, note: "dead team" }; }
  const book = await bookState(uid, team, s);
  const openSyms = new Set(book.openTrades.map((t) => t.symbol));
  const wants = decisions.map((d) => { const st = ((d.brief as J).setup as J) ?? {}; return { symbol: String(st.symbol), venue: String(st.venue), instrument: String(st.instrument), timeframe: String(st.timeframe) }; });
  const [cache, fundR, feed] = await Promise.all([
    s.research === "light" ? prefetchBars(uid, wants) : Promise.resolve({} as BarCache),
  ...
  for (const job of jobs) {
    const d = job.d, brief = d.brief as J, setup = brief.setup as J;
    const cost = job.ballots.reduce((a, b) => a + b.cost_usd, 0);
    const ballots: J[] = job.ballots.map((b) => ({ ...b }));
    let verdict: Verdict | null = verdicts[String(d.id)] ?? null;
    let outcome: Exec;
    if (openSyms.has(String(setup.symbol))) outcome = { taken: false, reasons: [`${setup.symbol} is already on the team's book`], pass_reason: "already held", by: "guardrail" };
    else if (!job.ballots.some((b) => b.stance === "take" && !b.error)) outcome = { taken: false, reasons: [job.ballots.every((b) => b.error) ? "no worker answered" : `${job.ballots.filter((b) => b.stance === "pass" && !b.error).length} of ${job.ballots.filter((b) => !b.error).length} workers said pass; the frontier was not asked`], pass_reason: job.ballots.every((b) => b.error) ? "no worker answered" : "every worker passed", by: "workers" };
      ...
      const q = await quotesFor(uid, [{ symbol: plan.symbol, venue: plan.venue }]);
      outcome = await execute(uid, acct, team, String(d.id), "candidate", plan, verdict, job.ballots, book, rules, s, num((setup.card as J)?.atr) || null, q[plan.symbol] ?? null, champion, Date.now());
      if (outcome.taken) openSyms.add(plan.symbol);
    }
    ...
    await finish(d, { status: "done", ballots, verdict: verdict ? { ...verdict } : null, outcome, cost_usd: round(cost, 5) });
    if (d.setup_id) await rest(`desk_setups?id=eq.${d.setup_id}&status=eq.league`, { method: "PATCH", body: JSON.stringify({ status: "league" }) });
    stats.decisions++; if (outcome.taken) stats.takes++; else stats.passes++;
    (out.decided as number)++; if (outcome.taken) (out.taken as number)++; (out.cost as number) += cost;
  }
...
/* ── sessions: the workers propose, the frontier reviews the book ─────── */
async function session(uid: string, body: J): Promise<J> {
  ...
  const live = (await loadTeams(uid)).filter((t) => t.status === "live");
  const done = rows(await rest(`desk_decisions?user_id=eq.${uid}&kind=eq.session&created_at=gte.${today}T04:00:00Z&select=team_id,brief`)).filter((d) => ((d.brief as J)?.session_key) === keyOf).map((d) => String(d.team_id));
  let launched = 0;
  for (const team of live) {
    if (done.includes(team.id)) continue;
    launch({ mode: "session_team", userId: uid, team_id: team.id, session_key: keyOf });
    launched++;
  }
...
async function sessionTeam(uid: string, body: J): Promise<J> {
  ...
  if (await spentToday(uid, today) >= s.budget_usd_day) {
    await rest("desk_decisions", { method: "POST", body: JSON.stringify({ user_id: uid, team_id: team.id, kind: "session", symbol: "", status: "done", brief: { session_key: sessionKey, note: "no session: today's league budget is spent" }, ballots: [], outcome: { taken: false, reasons: ["budget spent"], pass_reason: "budget spent", by: "budget" } }) });
    return { note: "budget" };
  }
  const rules = rulesFor(String(acct.preset ?? "aggressive") as "no_limits", (acct.rules as Partial<Rules>) ?? {});
  const book = await bookState(uid, team, s);
  ...
  const ideas = await Promise.all(team.workers.map(async (model) => {
    const res = await callModel(key, { model, system: PROPOSE_SYSTEM(team, book, s), user, schema: PROPOSE_SCHEMA, maxTokens: 2500, deadline: Math.min(deadline - 60_000, t0 + 45_000), reasoning: workerReasoning(model) });
    const j = res.json ?? {};
  ...
  // the frontier reviews the book and judges the proposals
  const marks = book.openTrades.length ? await quotesFor(uid, book.openTrades.map((t) => ({ symbol: t.symbol, venue: t.venue }))) : {};
...
/* ── the daily cut and the councils ───────────────────────────────────── */
async function council(uid: string, body: J): Promise<J> {
  const t0 = Date.now();
  const acct = await loadAccount(uid);
  if (!acct) return { error: "no account" };
  const s = leagueSettings(acct.league as Partial<LeagueSettings>);
  ...
  if (!force && now.hour * 60 + now.minute < 16 * 60 + 6) return { note: "the cut is at 16:06 ET" };
  const doneToday = rows(await rest(`desk_roster_log?user_id=eq.${uid}&action=eq.council&at=gte.${today}T04:00:00Z&select=id&limit=1`)).length > 0;
  if (doneToday && !force) return { note: "the cut already ran today" };
  const all = await loadTeams(uid);
  if (!all.some((t) => t.status === "live")) return { note: "no live teams" };
  const season = await currentSeason(uid, s, today);
  const marks = await markTeams(uid, s, all, num(season.n, 1));
  const out: J = { day: today, died: marks.died, moved: [] as string[], cut: "", champion: "", councils: 0, passive: [] as string[] };
  ...
  // the worst in Bronze by ranked return, if it is at least a day old, dies; a passive team is cut before any active one
  const sc = (t: TeamRow) => num(t.stats.rank_score, t.return_pct);
  const bronze = all.filter((t) => t.status === "live" && t.tier === "bronze" && Date.parse(t.formed_at) < dayStart).sort((a, b) => sc(a) - sc(b));
  if (bronze.length) {
    const worst = bronze[0];
    const pd = num(worst.stats.passive_days);
    const reason = `worst in Bronze at the daily cut (${worst.return_pct >= 0 ? "+" : ""}${worst.return_pct.toFixed(2)}%${pd ? `, ranked ${sc(worst) >= 0 ? "+" : ""}${sc(worst).toFixed(2)}% after ${pd} passive day${pd === 1 ? "" : "s"}` : ""})`;
    const r = await dieTeam(uid, s, all, worst, reason, num(season.n, 1));
    out.cut = `${worst.name} (${reason})${r.replacement ? `; ${r.replacement.name} formed` : ""}`;
    await log(uid, worst.name, "cut", worst.frontier, r.replacement?.name ?? "", reason);
  }
  // the season
  ...
  // the councils, one child per team at least a day old
  for (const team of all.filter((t) => t.status === "live" && Date.parse(t.formed_at) < dayStart)) {
    launch({ mode: "council_team", userId: uid, team_id: team.id, day: today });
    (out.councils as number)++;
  }
  await log(uid, "the leagues", "council", "", "", `daily cut ran: ${(out.moved as string[]).length} moved, ${out.cut || "no cut"}, ${out.councils} councils`);
  out.ms = Date.now() - t0;
  return out;
}
async function councilTeam(uid: string, body: J): Promise<J> {
  const t0 = Date.now(), deadline = t0 + 120_000;
  const [acct, team] = await Promise.all([loadAccount(uid), loadTeam(uid, str(body.team_id, 64))]);
  if (!acct || !team || team.status !== "live") return { error: "no live team" };
  const s = leagueSettings(acct.league as Partial<LeagueSettings>);
  const day = /^\d{4}-\d{2}-\d{2}$/.test(String(body.day)) ? String(body.day) : etDate(t0);
  if (rows(await rest(`desk_councils?team_id=eq.${team.id}&day=eq.${day}&select=id&limit=1`)).length) return { note: "already held" };
  const key = (await secret("anthropic_api_key")) || ENV_KEY;
  if (!key) return { error: "no key" };
  const book = await bookState(uid, team, s);
  const members = [team.frontier, ...team.workers];
  const ratings = rows(await rest(`desk_ratings?user_id=eq.${uid}&model=in.(${members.map((m) => `"${m}"`).join(",")})&select=model,elo,n_sits,n_sit_right,brier_sum,brier_n`));
  ...
  const all = await loadTeams(uid);
  const liveTeams = all.filter((t) => t.status === "live");
  const onTeam = new Set(members);
  const sideline = [...s.frontier_pool, ...s.worker_pool].filter((m) => !onTeam.has(m)).map((m) => `${m} (${s.frontier_pool.includes(m) ? "frontier" : "worker"}, standing ${poolStanding(m, all.map(teamLike)) === null ? "untested" : `${(poolStanding(m, all.map(teamLike)) as number) >= 0 ? "+" : ""}${(poolStanding(m, all.map(teamLike)) as number).toFixed(2)}%`})`);
  const user = `THE TEAM\n${records.map((r) => `- ${r.model} (${r.role}): ${r.record}`).join("\n")}\n\nTHE SIDELINE (who could come in)\n${sideline.join("\n")}\n\n${bookText(book)}`;
  ...
  const votes = await Promise.all(voters.map(async (model) => {
    ...
    const res = await callModel(key, { model, system: COUNCIL_SYSTEM(team, book, role, s), user, schema: COUNCIL_SCHEMA, maxTokens: 2500, deadline: Math.min(deadline - 15_000, t0 + 70_000), reasoning: role === "frontier" ? "low" : workerReasoning(model) });
  ...
  for (const v of votes) for (const x of v.votes) if (x.vote === "kick") kicks[x.member] = (kicks[x.member] ?? 0) + 1;
  const standing = (m: string) => poolStanding(m, all.map(teamLike)) ?? 0;
  const candidates = Object.entries(kicks).filter(([, n]) => n >= 2).sort((a, b) => b[1] - a[1] || standing(a[0]) - standing(b[0]));
  let kicked: string | null = null, replacedBy: string | null = null, reason = "";
  if (candidates.length) {
    const [m] = candidates[0];
    const why = votes.flatMap((v) => v.votes.filter((x) => x.member === m && x.vote === "kick").map((x) => `${v.model}: ${x.reason}`)).join(" | ");
    const used = new Set(all.map((t) => comboKey(t.frontier, t.workers)));
    if (m === team.frontier) {
      const led = (f: string) => liveTeams.filter((t) => t.frontier === f).length;
      const pick = s.frontier_pool.filter((f) => f !== m && led(f) < 2).sort((a, b) => standing(b) - standing(a) || led(a) - led(b)).find((f) => !used.has(comboKey(f, team.workers)));
      if (pick) { replacedBy = pick; await rest(`desk_teams?id=eq.${team.id}`, { method: "PATCH", body: JSON.stringify({ frontier: pick, combo: comboKey(pick, team.workers) }) }); }
    } else {
      const seats = (w: string) => liveTeams.filter((t) => t.workers.includes(w)).length;
      const others = team.workers.filter((w) => w !== m);
      const pick = s.worker_pool.filter((w) => !onTeam.has(w)).sort((a, b) => standing(b) - standing(a) || seats(a) - seats(b)).find((w) => !used.has(comboKey(team.frontier, [...others, w])));
      if (pick) {
        replacedBy = pick;
        const workers = [...others, pick];
        const seniors = team.seniors.filter((x) => workers.includes(x));
        while (seniors.length < 2) { const next = workers.find((w) => !seniors.includes(w)); if (!next) break; seniors.push(next); }
        await rest(`desk_teams?id=eq.${team.id}`, { method: "PATCH", body: JSON.stringify({ workers, seniors, combo: comboKey(team.frontier, workers) }) });
      }
    }
    if (replacedBy) { kicked = m; reason = why; await log(uid, team.name, "kicked", m, replacedBy, why); }
    else reason = `the council voted to kick ${m} (${why}) but every replacement would repeat a set of models already used`;
  } else reason = votes.every((v) => v.error) ? "no council member answered" : "no member drew two kick votes";
  const cost = votes.reduce((a, v) => a + v.cost, 0);
  await rest("desk_councils", { method: "POST", body: JSON.stringify({ user_id: uid, team_id: team.id, day, votes: votes.map((v) => ({ model: v.model, role: v.role, votes: v.votes, error: v.error })), kicked, replaced_by: replacedBy, reason, cost_usd: round(cost, 5) }) });
  await rest("desk_decisions", { method: "POST", body: JSON.stringify({ user_id: uid, team_id: team.id, kind: "council", symbol: "", status: "done", brief: { day, members: records }, ballots: [], verdict: null, outcome: { votes: votes.map((v) => ({ model: v.model, role: v.role, votes: v.votes })), kicked, replaced_by: replacedBy, reason }, cost_usd: round(cost, 5) }) });
  if (kicked) await bumpStats(team.id, { kicks: 1 });
  return { team: team.name, kicked, replaced_by: replacedBy, reason, cost: round(cost, 4), ms: Date.now() - t0 };
}

/* ── forming the tournament ───────────────────────────────────────────── */
async function form(uid: string): Promise<J> {
  ...
  for (const d of draftTeams(s)) {
    const t = await insertTeam(uid, s, all, { frontier: d.frontier, workers: d.workers, combo: d.combo }, d.tier, num(season.n, 1), "the opening draft");
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (mode === "session_team") return ok(internal ? await sessionTeam(uid, body) : { error: "children are launched by the desk" });
    if (mode === "council") return ok(await council(uid, body));
    if (mode === "council_team") return ok(internal ? await councilTeam(uid, body) : { error: "children are launched by the desk" });
    if (mode === "status") {
