...
${t.symbol} ${t.side}${perp ? ` ${t.leverage}x perp` : ""} · template ${t.template ? `${t.template} ${templateName(num(t.template))}` : "none"} · regime ${t.regime} · from ${from}${t.strategy ? (String(t.strategy).startsWith("own:") ? ` on its own idea, a playbook it named "${String(t.strategy).slice(4).replace(/-/g, " ")}"` : ` on a ${t.strategy} setup`) : ""} · a ${t.timeframe ?? "swing"} trade${t.horizon_hours ? ` on a ${t.horizon_hours}-hour clock` : ""}${closedBy}
...
async function coach(uid: string, key: string, today: string, acct: J): Promise<J> {
  ...
  const teamCards = liveOrDiedToday.map((t) => {
    ...
    const reads = mine.filter((d) => d.kind === "candidate" || d.kind === "session" || d.kind === "own");
    ...
    return {
      ...
      return_pct: num(t.return_pct), rank_score: num(stats.rank_score, num(t.return_pct)), passive_days: num(stats.passive_days), vests: num(stats.vests), cushion: num(stats.cushion), risk_bonus: num(stats.risk_bonus), shield: num(stats.shield), revivals: num(stats.revivals), own_takes: num(stats.own_takes), day_pnl: num(stats.day_pnl), equity: num(t.equity), days_alive: daysBetween(String(t.formed_at), today),
