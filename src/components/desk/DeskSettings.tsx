...
export default function DeskSettings({ uid, account, onSaved }: { uid: string; account: Account; onSaved: () => void }) {
  ...
  async function save() {
    ...
    const r = await updateAccount(uid, { preset, roster: models, judge: judge.trim(), budget_usd_per_run: budgetN, leverage_cap_override: levN, rules: nextRules, sit_roster: sitModels, sit_budget_usd: sitBudgetN, cooldown_hours: cooldownN });
    setSaving(false);
