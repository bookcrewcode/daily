...
// TradeCard — one team's trade, told as a lesson. Why it was taken (the
// thesis, the levels, every worker's ballot, the frontier's reason and the
// whole ticket), where it stands at live prices while it runs, what happened
// when it closed, and the micro review that judges the reasoning apart from
// the money. Paper money, said plainly; every label explained where it sits.
      ...
      {open && (
        <div className="mt-2.5 pt-2.5 border-t border-[var(--border-1)] rise-in space-y-3">
          <div>
            ...
            {t.falsifier && <p className="text-[11.5px] text-[var(--text-3)] mt-1 leading-snug"><span className="text-[var(--text-4)]">Wrong if:</span> {t.falsifier}</p>}
            <p className="mono text-[10px] text-[var(--text-4)] mt-1.5 leading-relaxed">
              planned entry {fmtPrice(t.entry_ref)} · stop {fmtPrice(t.stop)} · target {fmtPrice(t.target)}{t.liq_price ? ` · liquidation ${fmtPrice(t.liq_price)}` : ""}
              <br />{t.qty} {t.unit}{t.qty === 1 ? "" : "s"} · {fmtMoney(t.notional)} position{t.instrument === "crypto_perp" ? ` on ${fmtMoney(t.margin)} put up at ${t.leverage}x` : ""} · risk {t.risk_pct}% of the book · confidence {(t.confidence * 100).toFixed(0)}%
            </p>
            <p className="text-[10px] text-[var(--text-4)] mt-1 leading-relaxed">
              The stop is where the idea is wrong, the target where it is done. The distance from entry to stop is 1R — one unit of risk, {fmtMoney(risk * t.qty * (t.unit === "contract" ? t.contract_value : 1))} here — and every result below is counted in those units, so a small trade and a big one can be compared.
            </p>
            {ordered.length > 0 && (
              <div className="mt-2 space-y-1">
                <p className="mono text-[9px] uppercase tracking-widest text-[var(--text-4)]">What the team said · {takes} of {workers.length} workers voted to take it</p>
                {ordered.map((b, i) => (
                  <p key={i} className="text-[11px] leading-snug flex items-start gap-1.5">
                    ...
                    <span>
                      <span className="mono text-[9px]" style={{ color: b.stance === "take" ? "var(--ok)" : "var(--text-4)" }}>{b.stance} {(b.confidence * 100).toFixed(0)}%</span> <span className="text-[var(--text-3)]">{modelLabel(b.model)}:</span> {b.thesis}
                      {b.wrong_if ? <span className="text-[var(--text-4)]"> Wrong if {b.wrong_if}</span> : null}
                    </span>
                  </p>
                ))}
                <p className="text-[10px] text-[var(--text-4)] leading-relaxed">Each worker read the same brief and voted take or pass with a confidence, from 0 to 100 percent. The vote does not decide anything: the frontier does.</p>
              </div>
            )}
            {(frontierSaid || decision?.verdict) && (
              <p className="text-[11.5px] leading-snug mt-2">
                ...
                <span className="mono text-[10px]" style={{ color: "var(--neon)" }}>{decision?.verdict?.action ?? "take"}</span> {frontierSaid}
              </p>
            )}
            {t.ticket && (
              <div className="mt-2">
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setWhole(!whole)} className="mono text-[9px] text-[var(--neon)] active:scale-95">{whole ? "less" : "whole ticket"}</button>
                </div>
                <p className="text-[10px] text-[var(--text-4)] leading-relaxed mb-1">Everything that went into the order: the brief the team read, the numbers it wrote and the rules it was checked against.</p>
                <Ticket ticket={t.ticket} compact={!whole} />
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {running ? (
              ...
              <>
                ...
                <p className="text-[10px] text-[var(--text-4)] mt-1 leading-relaxed">Against at worst and for at best are how far the trade went the wrong way and the right way while it was open, in units of the risk taken. A trade that went 2R for before closing at 0.3R had the profit and gave it back.</p>
          ...
          <div>
            <button onClick={() => setChart(!chart)} className="mono text-[10px] text-[var(--neon)] active:scale-95">{chart ? "hide the chart" : "Chart"}</button>
            {chart && (
              <div className="mt-1.5 rise-in">
                ...
                <p className="text-[10px] text-[var(--text-4)] mt-1 leading-relaxed">The lines are the entry, the stop and the target the team wrote before it knew anything; the marks are where it actually got in and out.</p>
              </div>
            )}
          </div>
            ...
            <div>
              ...
              {text ? (
                <div className="rounded-lg bg-[var(--raised)] border border-[var(--border-1)] p-3 space-y-1.5">
                  ...
                  <p className="text-[10px] text-[var(--text-4)] leading-relaxed">Reasoning held or broke is about the thinking, not the money: a sound idea can lose and a sloppy one can win. Process is the grade for how the trade was run. The quadrant puts the two together.</p>
