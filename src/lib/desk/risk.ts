// The Desk — account-level risk: presets, halts, liquidation, the ladder. Pure.
// Split from rules.ts so the ledger engine can carry this without the
// proposal-level sizing code.
import type { PresetKey, Rules, Side, Trade } from "./types";
import { addDays, nextSessionDate } from "./clock";

export const PRESETS: Record<PresetKey, Rules> = {
  // Phase 5 opened the "what, when, how much" caps on every preset; only no_limits switches the halts off.
  ...
  // "No restrictions" (Ben, 2026-09-08): leverage to the venue maximum, no halts, no heat cap. Three rules survive
  // inside the guardrail itself: a stop on every trade, the stop inside the liquidation price, one position per symbol.
