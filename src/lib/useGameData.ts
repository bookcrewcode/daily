...
export function GameProvider({ uid, children }: { uid: string; children: React.ReactNode }) {
  ...
  const load = useCallback(async () => {
    ...
    const g: GameData = {
      ...
      questClaimCount: (questRows ?? []).filter((q) => q.quest_key !== "sweep" && q.quest_key !== "weekly_review" && !String(q.quest_key).startsWith("chest_") && !String(q.quest_key).startsWith("boss_") && !String(q.quest_key).startsWith("gstep_")).length,
      gigEarnings,
      focusMinutesList,
